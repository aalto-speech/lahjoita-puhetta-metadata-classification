...
def load_labels_trn(trn_path, label_path):
    label_array = []
    trn_array = []
    with open(trn_path, 'r', encoding='utf-8') as f:
        trn_data = f.readlines()
...
def word_to_idx(data, embeddings):
    ...
    for seq in range(len(data)):
        res.append(prepare_word_sequence(data[seq], embeddings))
    return res
...
def topic_to_idx(topics, topic2idx):
    ...
    temp_res = []
    for topic in topics:
        temp_res.append(topic2idx[topic])
...
def collate(list_of_samples):
    ...
    input_seqs, trn_seqs, topic = zip(*list_of_samples)
    ...
    return pad_input_seqs, input_seq_lengths, pad_trn_seqs, topic


#def collate(list_of_samples):
#    list_of_samples.sort(key=lambda x: len(x[0]), reverse=True)
#    
#    input_seqs, topic = zip(*list_of_samples)
#
#    input_seq_lengths = [len(seq) for seq in input_seqs]
#
#    padding_value = 0
...
#    topic = torch.LongTensor(topic)
#    
#    return pad_input_seqs, input_seq_lengths, topic
