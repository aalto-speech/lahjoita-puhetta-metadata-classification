...
def get_predictions(x_vector_model, batch_size, test_data, use_trn):
    ...
    for i in range(35, 36):
        ...
        for l, batch in enumerate(test_data):
            checkpoint = torch.load('weights/model_asr_trn/state_dict_' + str(i) + '.pt', map_location=torch.device('cpu'))
...
def get_predictions_by_topic(x_vector_model, batch_size, test_data, use_trn):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print('Evaluating...')
    ...
    for i in range(5, 6):
        ...
        if total_0 != 0: 
            print('total Eläimet: %.d   accuracy: %.4f' % (total_0, correct_0 / total_0))

        if total_1 != 0: 
            print('total K-18: %.d   accuracy: %.4f' % (total_1, correct_1 / total_1))
        
        if total_2 != 0: 
            print('total Kerro: %.d   accuracy: %.4f' % (total_2, correct_2 / total_2))

        if total_3 != 0: 
            print('total Kesä: %.d   accuracy: %.4f' % (total_3, correct_3 / total_3))
        
        if total_4 != 0: 
            print('total Luonto: %.d   accuracy: %.4f' % (total_4, correct_4 / total_4))

        if total_5 != 0: 
            print('total Lähelläni: %.d   accuracy: %.4f' % (total_5, correct_5 / total_5))

        if total_6 != 0: 
            print('total Mediataidot: %.d   accuracy: %.4f' % (total_6, correct_6 / total_6))
         
        if total_7 != 0: 
            print('total Urheilu: %.d   accuracy: %.4f' % (total_7, correct_7 / total_7))
