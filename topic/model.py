...
class XVectorModel(nn.Module):
    ...
    def forward(self, input_tensor, use_trn, trn_seqs=None):
        ...
        # process audio through TDNN 
        #input_tensor_tdnn = self.tdnn_1(input_tensor)
        #input_tensor_tdnn = self.tdnn_2(input_tensor_tdnn)
        #input_tensor_tdnn = self.tdnn_3(input_tensor_tdnn)
        #input_tensor_tdnn = self.tdnn_4(input_tensor_tdnn)
        #input_tensor_tdnn = self.tdnn_5(input_tensor_tdnn)

        #std, mean = torch.std_mean(input_tensor_tdnn, unbiased=False, dim=1)
