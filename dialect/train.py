...
def train(pairs_batch_train, pairs_batch_dev, pairs_batch_dev_acc, x_vector_model, criterion, optimizer, batch_size, device, use_trn):
    for epoch in range(1, 41):
        ...
        for l, batch in enumerate(pairs_batch_dev_acc):
            ...
            true_dialect.append(label_seqs.item())
            predicted_dialect.append(topk.item())
        
        
        accuracy = accuracy_score(true_dialect, predicted_dialect)
        ...
        with open('loss/model_audio_subset.txt', 'a') as f:
        ...
        }, 'weights/model_audio_subset/state_dict_' + str(epoch) + '.pt')
